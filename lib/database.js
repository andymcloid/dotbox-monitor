...
class Database {
    ...
    async seedDefaultCategories() {
        return new Promise((resolve, reject) => {
            ...
            this.db.get('SELECT COUNT(*) as count FROM categories', (err, row) => {
                ...
                if (row.count === 0) {
                    // Insert default categories (without specifying ID - let it auto-increment)
                    const defaultCategories = [
                        ...
                        {
                            name: 'Infrastructure',
                            description: 'Core infrastructure and servers',
                            icon: '🏗️',
                            color: '#8b5cf6'
                        },
                        {
                        ...
                        {
                            name: 'Security',
                            description: 'Security and SSL monitoring',
                            icon: '🔒',
                            color: '#f59e0b'
                        },
                        {
        ...
        return new Promise((resolve, reject) => {
            ...
            const sql = `
                WITH RECURSIVE time_buckets AS (
                    ...
                    SELECT datetime('now', '-${hours} hours') as bucket_time
                    UNION ALL
                    -- Generate subsequent buckets
                    SELECT datetime(bucket_time, '+${bucketMinutes} minutes')
                    FROM time_buckets
                ...
                SELECT 
                    ...
                    COALESCE(sd.avg_response_time, 0) as response_time,
                    COALESCE(sd.status, 'unknown') as status,
                    COALESCE(sd.data_points_in_bucket, 0) as data_points_in_bucket,
                    ${bucketMinutes} as bucket_size_minutes
                FROM time_buckets tb
                LEFT JOIN service_data sd ON tb.bucket_time = sd.bucket_time
    ...
    async updateCategory(id, updates) {
        return new Promise((resolve, reject) => {
            ...
            const sql = `UPDATE categories SET ${fields}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`;
    ...
    async deleteCategory(id) {
        return new Promise((resolve, reject) => {
            ...
            this.db.get('SELECT COUNT(*) as count FROM services WHERE category = (SELECT name FROM categories WHERE id = ?)', [id], (err, row) => {
                if (err) {
                    reject(err);
                    return;
                }
                ...
                if (row.count > 0) {
                    reject(new Error(`Cannot delete category: ${row.count} services are using this category`));
                    return;
                }
