...
class HealthCheckService {
    ...
    setBroadcastCallback(callback) {
        ...
        console.log('WebSocket broadcast callback set - real-time updates enabled');
    }
    ...
    async getServiceGraphData(serviceId, hours = 24, maxPoints = 100) {
        try {
            return await this.database.getServiceGraphData(serviceId, hours, maxPoints);
        } catch (error) {
            console.error(`[GraphData] Failed to get graph data for service ${serviceId}:`, error.message);
            return [];
        }
    }
